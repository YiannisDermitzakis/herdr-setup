...
<!-- fr:journal kind=decision scope=plan id=54eb204e55f3 created=2026-09-08T18:20:07 phase=9 -->
### 54eb204e55f3 · decision · Detection table stays hard-coded per-target; herdr integration status supplies the target list, not the status line's own path (phase 9)

hs_agent_detect_spec (lib/common.sh) is a small case statement mapping a known target to its command name and configuration directory, covering the six named in the phase brief (claude, codex, opencode, copilot, cursor, hermes) plus the two flagged mismatches (antigravity-cli -> ~/.gemini, kimi -> ~/.kimi-code). Considered and rejected: deriving the configuration directory from the status line's own path (e.g. /home/placeholder-user/.claude/hooks/herdr-agent-state.sh) instead of a table entry. That path is only present for an agent Herdr ALREADY calls current or outdated -- an absent agent's line carries a path too, but it points at where the hook WOULD go, which happens to be identical in shape, so this might have worked -- but parsing a directory out of that path string ties detection to Herdr's own hook-install layout (.../hooks/herdr-agent-state.sh, or .../agent/extensions/herdr-agent-state.ts for pi) rather than the agent's own configuration root, and that layout is Herdr's implementation detail, not a contract this tool was handed. The table is one line per known target, explicit, and grep-able; a target hs_agent_detect_spec has not learned yet is omitted rather than guessed, which is the same fail-closed instinct as the rest of this tool. herdr integration status still supplies the TARGET LIST and each target's STATE (P9.T1.S2's own requirement) -- only the how-to-detect-it half is local.

<!-- fr:journal kind=decision scope=plan id=2cefb94ef6ae created=2026-09-08T18:21:16 phase=9 -->
### 2cefb94ef6ae · decision · Feed is scoped to exactly the just-installed agents via a scratch adapters directory, not a new feed.py flag (phase 9)

The phase brief's "invoke feed for exactly those agents" needed a way to restrict lib/feed.py's run() to a subset of agents without teaching it a filter no other caller needs. lib/feed.py already discovers adapters by DIRECTORY CONTENT (discover(), phase 6), so hs_onboard_feed_adapters_dir (lib/common.sh) builds a throwaway directory holding only symlinks to the adapters for the agents onboard actually installed or refreshed, and cmd_onboard hands that to hs_feed via --adapters instead of $HS_ROOT/adapters. An agent already current is never in that directory and so never gets a feed probe, proven in tests/test_onboard_offer.sh by giving a current, adapter-equipped agent (opencode) a live fixture pane it would happily report if it were ever asked -- its probe log stays empty. Filtering by adapter.agent AFTER probing (a Python-side change to feed.py) was considered and rejected: it would still probe every adapter in the real directory regardless of scope, and the design doc's own "this tool runs seldom ... robustness wins over startup cost" makes that an acceptable but unnecessary trade against the simpler shell-side directory scoping, which also keeps the scoping decision in the same file (herdr-setup) that decided which agents to install.

<!-- fr:journal kind=finding scope=plan id=92593cc8a38d created=2026-09-08T18:27:47 phase=9 state=fixed -->
### 92593cc8a38d · finding [fixed] · tests/test_entrypoint.sh leaned on onboard's phase-1 stub to prove global flag propagation; real onboard broke it (phase 9)

Full-suite run after cmd_onboard replaced the stub: tests/test_entrypoint.sh had two blocks written against the stub's own behaviour -- an unconditional 'onboard is accepted and exits 0' loop, and a flags-before/flags-after/no-flags block that asserted the stub's own 'dry-run=N yes=N' echo text on stdout. Both broke: real onboard now gates on hs_require_socket like apply and feed (so it fails closed, exit 3, with no server reachable in that file's unsandboxed environment), and it no longer echoes anything. Fixed by (1) folding onboard's own hs_require_socket refusal into the same block as apply's and feed's, matching their exact assertion shape, and (2) replacing the flag-echo assertions with a small local sandbox (a plain-file socket, a fake herdr integration status fixture offering one absent agent, a config directory for it under the test's own sandboxed $HOME) that proves --dry-run and --yes are visible by their OBSERVABLE EFFECT on real onboard instead -- a '+ herdr integration install <target>' preview line and no actual herdr call under --dry-run, nothing at all under no flags. Left as tests/test_onboard_offer.sh's own header explains: the true single-question real-terminal y/n path is not exercised end to end anywhere in this phase, on purpose, matching the established precedent that hs_confirm's only other caller (tests/test_apply_config.sh's shrinking-write gate) leaves the same gap -- proving it needs an actual pty, which no test file in this suite sets up.

<!-- fr:journal kind=finding scope=plan id=47a98c97aedc created=2026-09-08T19:29:12 phase=10 state=fixed -->
### 47a98c97aedc · finding [fixed] · hs_confirm's real-terminal branch was untested anywhere in the suite -- closed by tests/test_confirm.py (phase 10)

Disclosed by phase 9 and repeated in the acceptance matrix's onboard row: hs_confirm's two callers (cmd_onboard, hs_apply_config's shrinking-write gate) are both tested with NO terminal at all, which takes the `[ ! -t 0 ]` refusal, never the `read`. A misread there -- an empty answer or end-of-input treated as yes -- would have passed every existing test and, on a real host, destroyed the operator's Herdr config or installed an integration nobody accepted. tests/test_confirm.py reuses tests/test_feed_report.py's TestAtARealTerminal pty machinery (open a pty, write an answer, wait for the process, then close) rather than inventing a second approach, driving `bash -c '. lib/common.sh; hs_confirm ...'` directly since hs_confirm has no Python side. Covers: y/yes and case variants accept; n/no/an empty answer/a nonsense answer decline; end-of-input (nothing typed before the pty closes) declines; and a plain pipe fed literal "yes", with no terminal at all, still declines -- proving the terminal check runs BEFORE any read, not after. One early version got the pty ordering wrong (closing the master immediately after writing killed the process via hang-up before it could read the answer, failing every accept case) -- fixed by waiting for the process to exit before closing, except for the end-of-input case, which needs the close to happen first since that IS what produces EOF on the slave side.

<!-- fr:journal kind=decision scope=plan id=1ab72250e738 created=2026-09-08T19:29:29 phase=10 -->
### 1ab72250e738 · decision · shellcheck cleanup: source= directives to resolve dynamic sourcing, disable= only with a comment naming the reason (phase 10)

The entrypoint and every test_*.sh file source lib/common.sh and tests/helpers/assert.sh through a variable ($repo_root, $test_dir), which shellcheck cannot follow statically -- SC1091 on every one of the 30-odd sourcing lines across 17 files. Fixed, not suppressed: a `# shellcheck source=<path relative to CI's invocation directory>` directive above each one, combined with running shellcheck with `-x`, so it actually opens and checks the sourced file's own content too. Zero real findings remained after that in lib/common.sh and adapters/; the entrypoint had two (an unused $t_how field, fixed by reading it into `_`; and word-splitting on $fed_agents, already correct and already commented, given the matching `# shellcheck disable=SC2086` directive). What remained after that was noise specific to this test suite's own idioms, each disabled with a comment naming why rather than rewritten: SC2015 (`A && pass || fail C`) is safe here because assert.sh's pass()/fail() never return nonzero -- one file-wide directive per affected file (7), not 39 individual rewrites of an established idiom; SC2012 (`ls -l | cut -c1-10` for a mode string) is deliberate portability, matching lib/common.sh's own `cp -p` choice to dodge the BSD/GNU `stat` flag split; SC2016 (single-quoted literal backticks) is single-quoting doing exactly its job; SC2329 (two test_public_hygiene.sh helpers 'never invoked') are invoked indirectly by name as absent()'s $lister argument, the exact case shellcheck's own message names as a possible false positive. One real fix landed outright: an `echo` with a literal \n in test_preflight.sh, portable as `echo` behaves today but ambiguous across shells, changed to `printf '%s\n'`. A directive with inline trailing text after the code list (`# shellcheck disable=SC2015  -- because...`) breaks shellcheck's own parser (SC1072/SC1073) -- the reason and the directive must be on separate comment lines, discovered when a first pass of this cleanup silently broke every one of those files' directives at once.

<!-- fr:journal kind=finding scope=plan id=ed4cee369c95 created=2026-09-08T19:29:42 phase=10 state=fixed -->
### ed4cee369c95 · finding [fixed] · test_entrypoint.sh's diff/absorb assertions assumed no manifest/ existed; seeding one for real broke them honestly (phase 10)

This phase seeds manifest/plugins.list and manifest/config.toml in the real checkout for the first time (P10.T1.S2). tests/test_entrypoint.sh drives the real entrypoint against this actual repository (not a sandbox copy) for exactly the commands safe to run that way, and two of its assertions were written when manifest/ did not exist yet: 'diff fails closed on a missing manifest' (expected exit 2) and 'absorb --dry-run against a clean checkout' (expected exit 0, the checkout being clean because there was nothing under manifest/ for git to see as dirty). With a real manifest now on disk, diff against the fresh, herdr-less $HOME every test file gets reports both seeded plugins missing and the seeded [ui] section as config drift -- exit 1, not 2 -- so that block now asserts drift instead of the missing-manifest error (which stays covered directly, against a manifest this suite deletes on purpose, by tests/test_diff_plugins.sh and tests/test_diff_config.sh). The absorb block needed no code change: 'clean' there means 'git status --porcelain -- manifest/ is empty', which is true once manifest/ is committed and was never going to be true mid-phase while it sat staged or untracked -- committing this phase's own changes is what makes that assertion true again, not a test edit.

<!-- fr:journal kind=finding scope=plan id=bd181db631fa created=2026-09-08T19:43:32 phase=10 state=fixed -->
### bd181db631fa · finding [fixed] · test_entrypoint.sh's absorb --dry-run assertion asserted manifest/ does not exist -- wrong once phase 10 seeded it for real (phase 10)
