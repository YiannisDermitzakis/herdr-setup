...
<!-- fr:journal kind=finding scope=plan id=p3-red-reached-real-fr created=2026-09-14T07:15:53 phase=3 state=fixed -->
### p3-red-reached-real-fr · finding [fixed] · The P3.T1.S1 RED run fell through PATH to the real fr and created a stray feat/x branch and workspace (phase 3)

Attributed to my own run, not a pre-existing state. With the fake fr not yet written, its test file's subprocess calls to a bare 'fr' resolved to the host's real fr. Its test_anything_else_is_refused case runs 'fr isolation up --branch feat/x' expecting the fake to refuse it; the real fr (cwd = this worktree, HOME = a mktemp -d I passed) instead created: (1) a branch refs/heads/feat/x at 08e2df1 (= main; no commits of its own) in the repository's shared git data, i.e. visible from the base clone; (2) a linked worktree at <that temp HOME>/.cache/fr/worktrees/herdr-setup/feat__x; (3) an fr workspace record: branch feat/x, profile host, no PR, no bound sessions. The suite run that followed only passed that subtest because the branch already existed, and tests/run.sh's repository-state check did NOT detect the damage: it fingerprinted HEAD and status --porcelain only, which a new branch or registered worktree does not change -- fixed by review item 4 (p3r-item4-repo-state), which adds for-each-ref and worktree list to the fingerprint. gh: the same RED run also resolved the real gh, but GH_TOKEN/GITHUB_TOKEN/GH_CONFIG_DIR were unset and HOME was a temp dir with no gh config, and auth status failed -- so no authenticated request was possible. Cleanup done by the orchestrator: fr isolation down --branch feat/x, then git branch -d feat/x. Prevention: the guard is feedlib.require_fakes, centralised in feedlib.isolate_environment for every Python test by review item 3 (p3r-item3-fakes-guard), with the same refusal in tests/run.sh before its loop and in tests/helpers/assert.sh for every shell test. It covers herdr and gh: the fake fr was removed with the fr enrichment (p3r-fake-fr-removed). State set to fixed by editing this entry in place, because fr journal add with an existing id is idempotent and changes nothing.
...
<!-- fr:journal kind=discovery scope=plan id=p3-red-t3s1 created=2026-09-14T08:00:10 phase=3 -->
### p3-red-t3s1 · discovery · RED: tests/test_audit_git.py before the git layer existed (P3.T3.S1) (phase 3)

Before lib/audit.py gained GitError, Repo, repo_for, local_ref, is_ancestor, local_default: uv run --quiet --script tests/test_audit_git.py -> rc=1, 16 tests, 21 errors (subtests counted), all AttributeError on the missing names: TestRepoFor (subdirectory -> toplevel/main checkout/slug; https with and without .git, git@github.com:, ssh://git@github.com/ all name example-org/example-repo; gitlab https, gitlab scp-like and a github.com.example.invalid lookalike have no slug but resolve; origin preferred; a sole non-origin remote used; several remotes without origin and no remote -> no slug; linked worktree names its main checkout; missing dir falls back; neither a work tree -> None), TestLocalRef (true/false, branch 123, 'feat' is not a prefix match of feat/x; a corrupt ref is GitError naming the repository, not False), TestIsAncestor (fresh and merge-commit-merged true, unmerged false; refs/remotes/origin/<default> wins over refs/heads/<default>; a corrupt ref is GitError naming the repository), TestLocalDefault (None without origin/HEAD, the target with it), TestNothingIsWrittenOrFetched (for-each-ref and status --porcelain of the repo and a linked worktree identical before and after; every git subcommand recorded by a PATH shim is in a read-only allowlist). Suite: /bin/bash tests/run.sh -> rc=1, 35/36 (FAIL: test_audit_git.py). Probe that shaped local_ref (scratch repo, not committed): for a corrupt loose ref, show-ref --verify --quiet and rev-parse --verify --quiet both exit 1 exactly as for a missing ref; for-each-ref refs/heads/<name> exits 0 with empty output but writes 'warning: ignoring broken ref' to stderr -- the only signal that tells broken from absent.

<!-- fr:journal kind=discovery scope=plan id=p3-mutation-github created=2026-09-14T08:04:08 phase=3 -->
### p3-mutation-github · discovery · Mutation evidence: -f variables, errors with exit 0, [bot] suffix, chunk size (P3.T2) (phase 3)

git archive 857957c into a fresh temp copy per mutation, replacement count asserted == 1, tests/test_audit_github.py run standalone from the copy (FAKE_GH_* cleared, temp HOME), copy removed; worktree untouched. (E) graphql() sends each variable with -F instead of -f -> rc=1: test_chunks_by_twenty_passes_names_with_dash_f_and_answers_per_name ERROR (branch 123 becomes an Int, the fake's String! coercion error raises GhError), test_every_open_pr_across_paginated_repos_and_pull_requests FAIL and test_returns_the_status_per_name_in_chunks FAIL (both via assert_no_typed_fields). (F) gh()'s errors check replaced by 'if False:' -> first run rc=1 with only the open_prs subtest FAILing plus an unlabeled ERROR: the test's assertIn on ctx.exception sat outside the subTest, so the first surviving call aborted the loop and repo_branches/compare were never exercised (finding p3-subtest-loop-aborts, fixed). Re-run with the restructured test file copied into the 857957c archive: rc=1, 3 failures -- test_errors_returned_with_exit_zero_raise_quoting_the_message for compare, open_prs and repo_branches. (G) bot rule reduced to __typename == 'Bot' -> rc=1, 1 failure: test_bots_by_typename_or_login_suffix_and_a_deleted_author_is_not_one. (H) BRANCH_CHUNK = 25 -> rc=1, 2 failures: test_chunks_by_twenty_passes_names_with_dash_f_and_answers_per_name, test_returns_the_status_per_name_in_chunks.

<!-- fr:journal kind=finding scope=plan id=p3-subtest-loop-aborts created=2026-09-14T08:04:10 phase=3 state=fixed -->
### p3-subtest-loop-aborts · finding [fixed] · Three GitHub-layer loop tests stopped at the first surviving call (phase 3)

test_a_login_that_cannot_be_a_github_login_is_refused_by_name, test_a_non_zero_gh_exit_raises_quoting_gh and test_errors_returned_with_exit_zero_raise_quoting_the_message used 'with self.subTest(...), self.assertRaises(...) as ctx:' and then read ctx.exception OUTSIDE the subTest. When a call did not raise, the subTest recorded a failure and the next line raised AttributeError, which ended the whole test -- so later calls in the loop were never checked. Mutation F showed it: only 1 of 3 calls reported. Fixed by nesting assertRaises and the message check inside each subTest; the re-run mutation reports all 3.

<!-- fr:journal kind=discovery scope=plan id=p3-green-t3s2 created=2026-09-14T08:10:50 phase=3 -->
### p3-green-t3s2 · discovery · GREEN: the git layer in lib/audit.py (P3.T3.S2) (phase 3)

lib/audit.py gains GitError, Repo(toplevel, main_checkout, slug), github_slug (three whole-match regexes: https with optional credentials, git@github.com:, ssh://git@github.com[:port]/), repo_for (dir, else fallback; a missing or non-work-tree directory is None, never an error), _main_checkout (first 'worktree' line of worktree list --porcelain), _slug (origin, else the only remote, via remote get-url), _ref_exists/local_ref (for-each-ref; any stderr is GitError, because a broken ref otherwise reads as absent), is_ancestor (merge-base --is-ancestor against refs/remotes/origin/<default> when it exists, else refs/heads/<default>; 0 true, 1 false, anything else GitError naming the repository), local_default (symbolic-ref --quiet refs/remotes/origin/HEAD; 1 -> None). Every call is git -C <dir> with GIT_OPTIONAL_LOCKS=0. tests/test_audit_git.py rc=0 16/16; tests/test_audit_github.py rc=0 26/26 after the loop fix; /bin/bash tests/run.sh rc=0 36/36; ruff check and format --check rc=0.

<!-- fr:journal kind=discovery scope=plan id=p3-norefactor-t3s3 created=2026-09-14T08:10:52 phase=3 -->
### p3-norefactor-t3s3 · discovery · no-refactor-because: P3.T3.S3 (phase 3)

The layer is already one function per git question -- which work tree (_toplevel), which main checkout (_main_checkout), which slug (_slug), does a ref exist (_ref_exists, with local_ref its refs/heads/ form), is it contained (is_ancestor), what is origin/HEAD (local_default) -- each running exactly one git subcommand (is_ancestor two: the default-ref existence check it needs, then merge-base), sharing only _git (the subprocess call) and _git_failure (the message). Nothing duplicated to fold. Suite rc=0 36/36.

<!-- fr:journal kind=discovery scope=plan id=p3-mutation-git created=2026-09-14T08:19:32 phase=3 -->
### p3-mutation-git · discovery · Mutation evidence: broken-ref detection, remote-tracking default, cwd fallback, origin preference (P3.T3) (phase 3)

git archive af15808 into a fresh temp copy per mutation, replacement count asserted == 1, tests/test_audit_git.py run standalone from the copy, copy removed; worktree untouched. (I) _ref_exists no longer treats stderr as an error (returncode only) -> rc=1, 1 failure: test_a_ref_git_cannot_read_is_an_error_not_an_absence (a corrupt ref read as absent). (J) is_ancestor always compares against refs/heads/<default> -> rc=1, 1 failure: test_the_remote_tracking_default_wins_over_the_local_one_when_it_exists. (K) repo_for tries only the directory, never the fallback -> rc=1, 2 errors: test_a_missing_directory_falls_back_to_the_fallback, test_refs_and_status_are_identical_and_only_read_only_subcommands_ran (its removed-directory entry resolves to None). (L) _slug takes the first remote whenever there is one, origin not preferred -> rc=1, 2 failures: test_origin_is_preferred_over_another_remote, test_several_remotes_and_no_origin_or_no_remote_at_all_mean_no_slug.

<!-- fr:journal kind=discovery scope=plan id=p3-red-t4s1 created=2026-09-14T08:25:50 phase=3 -->
### p3-red-t4s1 · discovery · RED: tests/test_audit_merge_state.py before resolve_branches existed (P3.T4.S1) (phase 3)

First attempt did NOT count as RED: all 17 tests errored with FileNotFoundError because auditlib.FakeGh wrote its state into a directory it never created (the test put it at <tmp>/gh). A RED that fails on its own setup proves nothing about the missing code, so auditlib.FakeGh now creates its directory, and both runs were repeated. Counted RED: uv run --quiet --script tests/test_audit_merge_state.py -> rc=1, 17 tests, 17 errors, every one "AttributeError: module 'hs_audit' has no attribute 'resolve_branches'": TestTheTable (squash-merged -> merged with pr; merge-commit merged, no PR -> contained; fresh branch -> contained; local-only unpushed -> unmerged; GitHub-only ref AHEAD -> unmerged, BEHIND/IDENTICAL -> contained; contained local ref + GitHub AHEAD -> unmerged; OPEN PR -> open-pr with number and draft; no ref and only CLOSED or no PR -> gone; a fork's MERGED and OPEN PRs and a MERGED PR with headRepository null -> not used, unmerged; branch 123 -> merged), TestFindingTheRepository (missing dir resolves through the session cwd; no work tree -> unresolved with a reason and no gh call; non-GitHub without origin/HEAD -> unresolved, no gh call; non-GitHub with origin/HEAD -> ancestry only: contained/unmerged/gone), TestWhatIsAsked (HsCompare asked exactly once with only the name whose GitHub ref exists and no PR decided; four entries naming one branch from the checkout, a subdirectory, a linked worktree and a removed dir -> one resolution, one HsRepoBranches call with one h variable, all four (dir, cwd) sources recorded, refs and status unchanged, no mutation; a corrupt ref raises GitError). Suite: /bin/bash tests/run.sh -> rc=1, 36/37 (FAIL: test_audit_merge_state.py).

<!-- fr:journal kind=discovery scope=plan id=p3-green-t4s2 created=2026-09-14T08:34:32 phase=3 -->
### p3-green-t4s2 · discovery · GREEN: merge state and resolve_branches (P3.T4.S2) (phase 3)

lib/audit.py gains the state names, Facts (None = not asked), Resolution(state, pr, repo_slug, reason, sources), pr_decision (a pull request of THIS repository: head_repo compared case-insensitively, null head_repo never matches; MERGED before OPEN, highest number wins), classify (the table, pure; raises ValueError when a fact it needs was never gathered), _resolve_repository (HsRepoBranches first; then, only for names no pull request decided and only with a default branch, local_ref, is_ancestor where the local ref exists, and compare for those whose GitHub ref exists), resolve_branches (locates each distinct (dir, cwd) once, groups by main checkout, keys (main_checkout, name); an entry with no work tree is unresolved under its own dir). tests/test_audit_merge_state.py rc=0 17/17; /bin/bash tests/run.sh rc=0 37/37; ruff check and format --check rc=0. Deviation from the plan's signature, deliberate: Resolution carries a fifth field, sources -- the distinct (dir, cwd) pairs that named the branch -- because the plan's return type {(repo_key, name): Resolution} gives phase 4 no way to map a session's (name, dir) back to its key without re-running repo_for; with sources it is a dictionary lookup. The key for a no-work-tree entry is (its dir, name), since there is no repository to key it by.

<!-- fr:journal kind=discovery scope=plan id=p3-mutation-merge-state created=2026-09-14T08:47:38 phase=3 -->
### p3-mutation-merge-state · discovery · Mutation evidence: PR-first, head repository, compare only on existing refs, BEHIND, resolve once (P3.T4) (phase 3)

git archive b7c7798 into a fresh temp copy per mutation with the CURRENT tests/test_audit_merge_state.py (including the pure table) copied in, replacement count asserted == 1, test run standalone from the copy, copy removed; worktree untouched. (M) classify's pull request decision replaced by None -> rc=1, 9 errors: test_squash_merged_is_merged_though_its_commits_are_not_ancestors, test_an_open_pull_request_is_open_pr_with_its_number_and_draft, test_a_branch_named_123_resolves, test_compare_is_asked_only_for_existing_github_refs_no_pull_request_decided, test_the_chosen_pull_request_is_the_highest_numbered_of_its_state, and 4 rows of test_every_row_of_the_table (the classify guard raised for a fact the PR rows never needed). (N) the head-repository check replaced by 'if True' -> rc=1, 6 failures: test_a_forks_same_named_pull_request_is_not_this_repositorys for fork-merged, fork-open and deleted-fork, and the table rows for a fork's merged PR, a fork's open PR and a null head repository. (O) compare asked for every undecided name, not only existing GitHub refs -> rc=1, 8 errors (GhError on the fake's NOT_FOUND for every branch without a GitHub ref, and the compare-asked test). (P) CONTAINED_COMPARE without BEHIND -> rc=1, 2 failures: test_a_github_only_ref_is_decided_by_compare and the table row 'a GitHub ref BEHIND'. (Q) resolve_branches' per-repository name dedupe removed -> SURVIVED at first (rc=0, 20/20): repo_branches deduplicates on its own and the result dict collapses duplicates, so one answer and one HsRepoBranches call still came back, while local_ref and is_ancestor ran once per duplicate entry. Test strengthened (finding p3-resolve-once-uncounted); re-run with the strengthened test: rc=1, 1 failure, test_one_resolution_per_repository_and_branch_however_many_name_it: 'for-each-ref refs/heads/feat/shared' ran 4 times, expected 1.

<!-- fr:journal kind=finding scope=plan id=p3-resolve-once-uncounted created=2026-09-14T08:47:40 phase=3 state=fixed -->
### p3-resolve-once-uncounted · finding [fixed] · 'One resolution per (repo, branch)' was tested by the answer, not by the work (phase 3)

The test asserted one key in the result and one HsRepoBranches call with one variable. Mutation Q (no dedupe in resolve_branches) left both true -- repo_branches dedupes its own input -- while the local ref lookup and merge-base ran once per entry naming the branch. The spec says several sessions and directories naming the same branch 'resolve once'. The test now runs resolve_branches under the recording git shim and requires exactly one 'for-each-ref ... refs/heads/feat/shared' and one merge-base for it across the four entries; the mutation now fails 4 != 1.

<!-- fr:journal kind=discovery scope=plan id=p3-refactor-t4s3 created=2026-09-14T08:47:42 phase=3 -->
### p3-refactor-t4s3 · discovery · refactor-done: P3.T4.S3 (classify pure, table test added) (phase 3)

classify(Facts) was written as a pure function of gathered facts in GREEN (Facts holds prs, local_ref, local_contained, remote_ref, remote_status, with None meaning not asked; all git and gh calls live in _resolve_repository). This step added TestClassifyIsAPureTable to tests/test_audit_merge_state.py: 21 rows of the table from Facts alone with subprocess.run patched to raise (so any git or gh call fails the row), the highest-numbered pull request of the deciding state is the one reported, and a needed fact that was never gathered raises ValueError. The table test is part of the mutation evidence in p3-mutation-merge-state (M, N and P each fail table rows). The one-resolution test was strengthened in the same step (p3-resolve-once-uncounted).

<!-- fr:journal kind=discovery scope=plan id=p3-quality-gate-t5s1 created=2026-09-14T09:06:59 phase=3 -->
### p3-quality-gate-t5s1 · discovery · Quality gate: lib/audit.py read against 'The audit runner' (P3.T5.S1) (phase 3)

Read lib/audit.py end to end against the spec's Resolving a branch, Merge state, GitHub queries, Bots and Errors and safety. Drift found and fixed: (1) the module docstring still called the git layer and merge state future work -- it now lists the three layers and says the sources, join and report are phase 4 and main() calls none of it yet; (2) the spec's GitHub queries section did not say every call names --hostname github.com, nor that HsOwnerPullRequests' nested pullRequests selects pageInfo -- both now stated in the spec, with the reason; (3) open_prs' docstring now says each record's repo carries the login as given, so phase 4 matches slugs ignoring case. Confirmed without change: dir-else-cwd, a missing directory is not a work tree, origin-else-sole-remote on github.com, key = (main checkout, branch), GitHub's default else origin/HEAD else unresolved, local ancestry against refs/remotes/origin/<default> else refs/heads/<default> with exit 0/1/other, compare IDENTICAL or BEHIND is contained and asked only for existing refs no PR decided, PR check before ancestry with head repository matched and null head repository unused, any gh error or non-zero exit fails, an unresolvable --owner names the login, bots by __typename Bot or [bot] suffix and a null author is not one. Out of phase 3's scope and untouched: Sources 1-4 (panes, session history, pane directories, fr bindings). Acceptance: audit-merge-state-sees-squash-merges gains levels.unit tests/test_audit_merge_state.py and notes on what phase 3 verified, and stays not-implemented (gone counted not listed, and neither reported as work, are the report's -- phase 4); audit-fails-closed gains levels.unit tests/test_audit_github.py and tests/test_audit_git.py and a phase 3 note, and stays not-implemented (the exit-2 wiring is phase 4). Reports regenerated with fr acceptance report --deterministic. Gates, each rc read directly: /bin/bash tests/run.sh rc=0 37/37; uv run --group dev pytest rc=0 477 passed; ruff check . rc=0; ruff format --check . rc=0 (50 files); shellcheck -s bash -x herdr-setup lib/*.sh tests/*.sh rc=0; fr acceptance check rc=0 (13 rows: 8 ci, 1 skipped, 4 not-implemented); git merge-base --is-ancestor origin/feat/session-audit HEAD rc=0.

<!-- fr:journal kind=discovery scope=plan id=p3r-item1-missing-default created=2026-09-14T10:04:50 phase=3 -->
### p3r-item1-missing-default · discovery · Review item 1: a default branch absent locally no longer aborts the audit (phase 3)

Wrong: is_ancestor fell back to refs/heads/<default> without checking it existed, so a pre-rename, --single-branch or fork clone made merge-base exit 128 -> GitError -> exit 2 for the whole run. Changed: default_ref() returns refs/remotes/<remote>/<default>, else refs/heads/<default>, else None; is_ancestor returns None when there is none; Facts.local_default_missing makes that branch unresolved with reason "default branch <name> not present locally". GitError stays for real failures on refs that exist. Spec Merge state bullet amended. RED: tests/test_audit_git.py rc=1 (the new none-not-an-error test ERROR) and tests/test_audit_merge_state.py rc=1 (the master-only clone raised GitError). GREEN rc=0 both. Commit eb8e9c9.

<!-- fr:journal kind=discovery scope=plan id=p3r-item2-chosen-remote created=2026-09-14T10:04:52 phase=3 -->
### p3r-item2-chosen-remote · discovery · Review item 2: slug and ancestry now read the same chosen remote (phase 3)

Wrong: the slug used origin-else-sole-remote while ancestry and origin/HEAD were hard-coded to origin. Changed: Repo carries remote (_remote_and_slug), and default_ref/local_default read refs/remotes/<remote>/... . Spec Resolving a branch and Merge state now say the chosen remote. RED: tests/test_audit_merge_state.py rc=1 -- sole remote upstream with upstream/main containing the branch and local main stale gave unmerged, not contained; with no local main it raised GitError; tests/test_audit_git.py rc=1 (Repo had no remote; upstream/HEAD read as None). GREEN rc=0. Commit eb8e9c9.

<!-- fr:journal kind=discovery scope=plan id=p3r-item3-fakes-guard created=2026-09-14T10:04:54 phase=3 -->
### p3r-item3-fakes-guard · discovery · Review item 3: one fakes guard for Python, and run.sh and assert.sh refuse too (phase 3)

Wrong: the guard lived only in auditlib plus two per-file copies; feedlib-only Python tests, tests/run.sh and every shell test could fall through PATH to the real herdr/gh/fr. Changed: feedlib.require_fakes (the one implementation) runs at the end of isolate_environment; the auditlib and per-file copies are removed; tests/run.sh refuses before its loop unless herdr, gh and fr resolve to $helpers_dir/<cmd>; tests/helpers/assert.sh refuses on source unless each resolves to a tests/helpers fake (any tests/helpers, because the hygiene tests source a nested copy while PATH still points at the real suite); tests/test_harness.sh copies all three fakes into its nested suite. RED on an ef5f3f7 archive copy with the gh symlink removed: test_feed_probe.py rc=0 (ran 20 tests), tests/run.sh with one test left rc=0 (1/1 passed), test_env_isolation.sh ran its assertions instead of refusing. Proof on an e361547 archive copy, gh symlink removed: (a) test_feed_probe.py rc=1 with RuntimeError gh resolves to <host gh>, not tests/helpers/fake-gh at import, before any test ran; (b) tests/run.sh rc=2 with tests/run.sh: refusing to run: gh does not resolve to the fake in tests/helpers, no test file run; (c) bash tests/test_env_isolation.sh rc=2 with refusing to run: gh does not resolve to the fake in tests/helpers. Commit e361547.

<!-- fr:journal kind=discovery scope=plan id=p3r-item4-repo-state created=2026-09-14T10:04:56 phase=3 -->
### p3r-item4-repo-state · discovery · Review item 4: run.sh repository fingerprint now includes refs and worktrees (phase 3)

Wrong: repo_state was HEAD plus status --porcelain, which neither a created branch nor a registered worktree changes -- exactly the real-fr incident. Changed: repo_state adds git for-each-ref and git worktree list --porcelain. No existing test writes a ref in the checkout (they build sandbox repos); tests/test_harness.sh passes (13 passed). RED on an ef5f3f7 archive copy turned into a sandbox git repo, with a throwaway tests/test_zz_probe.sh running git branch zz-probe against it: rc=0, 2/2 test files passed, branch created unnoticed. Proof on e361547, same probe: rc=1, FAIL: test_zz_probe.sh modified the repository it is testing, 1/2 test files passed. Commit e361547.

<!-- fr:journal kind=discovery scope=plan id=p3r-item5-pr-paging created=2026-09-14T10:18:42 phase=3 -->
### p3r-item5-pr-paging · discovery · Review item 5: a branch s pull requests are paged instead of silently truncated (phase 3)

Wrong: HsRepoBranches asked pullRequests(headRefName, first: 20) with no pageInfo and repo_branches forced hasNextPage=False, so a name shared by more than 20 OPEN/MERGED pull requests (patch-1 across forks) could push this repository s own MERGED one off the page. Changed: the per-branch selection adds pageInfo, and a branch with more pages is paged TO THE END with the new named query HsBranchPullRequests($owner, $name, $head, $after) -- to the end rather than until the first own pull request, because a later own MERGED pull request would change which one is reported. Spec GitHub queries item 3 names it. The fake already pages every connection at FAKE_GH_PAGE_SIZE with per-connection cursors, so it needed no change. RED: tests/test_audit_github.py rc=1 (test_hs_repo_branches ERROR, no pageInfo; test_a_branchs_pull_requests_are_paged_to_the_end FAIL); tests/test_audit_merge_state.py rc=1 (at FAKE_GH_PAGE_SIZE=1 a fork s MERGED #1 hid our MERGED #2: TypeError, no pr). GREEN rc=0 both.

<!-- fr:journal kind=discovery scope=plan id=p3r-item6-remote-shapes created=2026-09-14T10:18:43 phase=3 -->
### p3r-item6-remote-shapes · discovery · Review item 6: more github.com remote URL shapes are recognised (phase 3)

Wrong: only https, git@github.com: and ssh://git@github.com/ were read. Changed: ssh:// with or without a user and port, scp-like with or without any user before @, https with or without credentials, any letter case, .git with or without a trailing slash. An SSH host alias (git@github-work:o/r) stays unrecognised -- that needs ssh config -- and the spec Resolving bullet names it a known limit. RED: tests/test_audit_git.py rc=1, 3 failures: github.com:o/r, org-123@github.com:o/r.git, ssh://github.com/o/r. The uppercase-host, trailing-slash and git@GitHub.com cases passed already (re.I was present); they now guard re.I, whose removal survived a mutation before.

<!-- fr:journal kind=discovery scope=plan id=p3r-item7-toplevel-failures created=2026-09-14T10:18:45 phase=3 -->
### p3r-item7-toplevel-failures · discovery · Review item 7: only not-a-repository counts as not a work tree (phase 3)

Wrong: _toplevel read every rev-parse failure as not a work tree, so a safe.directory refusal or an unreadable repository was silently unresolved. Changed: a missing directory, or exit 128 saying not a git repository / must be run in a work tree (a bare repository), is None; any other failure in an existing directory is GitError naming it. _git runs with LC_ALL=C so those messages stay in English on a localised host. Spec Resolving bullet 1 amended. RED: tests/test_audit_git.py rc=1 (a repository with core.repositoryformatversion 99 returned None instead of raising). GREEN rc=0.

<!-- fr:journal kind=discovery scope=plan id=p3r-item8-fake-gh-details created=2026-09-14T10:18:47 phase=3 -->
### p3r-item8-fake-gh-details · discovery · Review item 8: fake gh infers ASCII digits only and checks the mutation tripwire first (phase 3)

Changed: infer() matches [+-]?[0-9]+ (Arabic-Indic digits stay a string, as with Go strconv); the mutation tripwire moved to main(), before FAKE_GH_FAIL and every other switch; the header names FAKE_GH_GRAPHQL_ERRORS as a construction the spec requires (real gh exits 1 on errors); tests/fixtures/gh/README.md no longer says a later phase. RED: tests/test_fake_gh.py rc=1 (test_dash_capital_f_infers_only_ascii_digits_like_go_strconv FAIL; test_the_mutation_tripwire_fires_before_fake_gh_fail FAIL, FAKE_GH_FAIL answered first). GREEN rc=0.

<!-- fr:journal kind=discovery scope=plan id=p3r-item9-auth-active created=2026-09-14T10:18:48 phase=3 -->
### p3r-item9-auth-active · discovery · Review item 9: the preflight checks only the active github.com account (phase 3)

Checked read-only on this host: gh 2.100.0 auth status --help lists -a, --active (Display the active account only). Changed: require_tools runs gh auth status --hostname github.com --active, so a stale inactive second account does not fail the preflight; the fake accepts --active/-a; spec audit step 2 amended. RED: tests/test_fake_gh.py rc=1 (the fake refused --active) and tests/test_audit_github.py rc=1 (require_tools did not send it). GREEN rc=0.

<!-- fr:journal kind=discovery scope=plan id=p3r-item10-resolution-index created=2026-09-14T10:18:50 phase=3 -->
### p3r-item10-resolution-index · discovery · Review item 10: resolutions carry an index from every entry to its key (phase 3)

Changed: resolve_branches returns Resolutions, a dict of {(repo_key, name): Resolution} with .index {(name, dir, cwd): key} and key_for(name, dir, cwd); dir and cwd are normalised to str(Path(p)), so a Path, a str and a trailing-slash str name one entry. RED: tests/test_audit_merge_state.py rc=1 (AttributeError: dict has no index). GREEN rc=0.

<!-- fr:journal kind=discovery scope=plan id=p3r-correction-sources-lookup created=2026-09-14T10:18:52 phase=3 -->
### p3r-correction-sources-lookup · discovery · Correction to p3-green-t4s2: sources alone were a scan, not a dictionary lookup (phase 3)

p3-green-t4s2 said Resolution.sources made mapping a session back to its key a dictionary lookup. It did not: a caller would have had to scan every resolution s sources for its (dir, cwd). The lookup exists now as Resolutions.index / key_for (p3r-item10-resolution-index). The original entry is left as written.

<!-- fr:journal kind=discovery scope=plan id=p3r-fake-fr-removed created=2026-09-14T10:18:54 phase=3 -->
...
<!-- fr:journal kind=discovery scope=plan id=p3s-suite-speed created=2026-09-14T12:34:46 phase=3 -->
### p3s-suite-speed · discovery · Suite speed: fake gh in-process for unit tests, template repo copies (phase 3)

Measured on this host, one run each with nothing else running (this host is slower than the orchestrator run: 1483 s here vs its 848 s). BEFORE at ade7998: uv run --group dev pytest -q --durations=30 -> rc=0, 483 passed, real 1482.96 s; /bin/bash tests/run.sh -> rc=1, 35/36, real 1837.35 s -- the one failure was test_contract.py: the copilot adapter probe exceeded its 10 s PROBE_TIMEOUT under the load of cold uv starts (a timing flake of the slow host, not a phase 3 assertion). Slowest before: test_audit_merge_state TestTheTable squash-merged 33.51 s, merge-commit contained 24.05 s; test_audit_git TestRepoFor every remote url form 20.45 s; merge_state missing directory via cwd 18.70 s, gone 18.66 s, non-github with origin/HEAD 18.13 s, local-only unmerged 17.31 s, sole remote stale local main 16.51 s; test_audit_git nothing written 15.42 s; test_contract empty store 14.56 s; test_audit_github chunks by twenty 14.45 s. Cause: every fake-gh call cold-started uv + Python + the fake parser through PATH, and every make_repo spawned init, add, commit, rev-parse (process start is slow on this host: the git-only url test took 20 s). Changed: tests/helpers/fake-gh gains a pure handle(argv, env, stdin) -> (stdout, stderr, code) that its CLI now runs (switches read from an injected ENV); lib/audit.py sends every gh call through one module-level gh_runner (default run_gh_process, the subprocess call) used by _run_gh and require_tools; tests/helpers/auditlib.py adds load_fake_gh, in_process_gh and use_in_process_gh (calls require_fakes, then patches audit.gh_runner), and make_repo copies a template repository built once per process (each test still owns its copy). test_audit_github and test_audit_merge_state run in-process; TestRequireTools keeps the subprocess runner (PATH smoke) and adds test_the_in_process_runner_answers_exactly_as_the_subprocess_runner (an HsRepoBranches query and a refused query, identical stdout, stderr and rc both ways). test_fake_gh keeps the CLI contract through PATH (REST, -F typing, failure switches, missing-ref bytes); TestCapturedShapes (except the missing-ref test) and TestQuerySemantics run the same handler in-process. No assertion changed; git test classes all mutate their repos, so no setUpClass sharing (template copy instead); no duplicate scenarios found. AFTER at d8e68c1: pytest -> rc=0, 484 passed, real 274.45 s (slowest now test_audit_git nothing written 5.85 s, test_contract empty home 5.25 s); tests/run.sh -> rc=0, 36/36, real 406.99 s. pytest misses the 3-minute target on this host by about 95 s; the remaining slow tests are mostly pre-existing adapter/feed subprocess tests (test_contract conformance, test_feed_report terminal tests), outside this brief.

<!-- fr:journal kind=discovery scope=plan id=p4-probe-timeout created=2026-09-14T12:52:58 phase=4 -->
### p4-probe-timeout · discovery · Real-adapter tests wait TEST_ADAPTER_TIMEOUT, not the production probe limit (phase 4)

Before any P4 task. tests/helpers/feedlib.py gains TEST_ADAPTER_TIMEOUT = 120.0. Every feed.probe/feed.resolve call on a REAL adapter in tests/test_adapter_{claude,codex,copilot,opencode}.py (7+5+6+6 probes, 13+12+13+13 resolves), the conformance probes and _run_sessions in tests/test_contract.py, and run_sessions plus the direct probe/bogus subprocess calls in tests/test_adapter_{claude,codex}_sessions.py now pass it. Left as they were: the documented minimal shell adapter in test_contract.py, and the deliberate small timeouts in test_feed_probe.py (0.5), test_feed_resolve.py (0.5) and test_feed_sessions.py (0.2). lib/feed.py's PROBE_TIMEOUT/RESOLVE_TIMEOUT/SESSIONS_TIMEOUT are unchanged. No RED is possible for a load-dependent flake; the seven edited files pass standalone (22, 19, 21, 21, 105, 28, 28 tests, rc=0). Baseline before phase 4 at 6b02f55, each run alone on this host: uv run --group dev pytest rc=0, 484 passed, real 214.82 s; /bin/bash tests/run.sh rc=0, 36/36, real 326.62 s.

<!-- fr:journal kind=discovery scope=plan id=p4-shellcheck-sc2015 created=2026-09-14T12:54:05 phase=4 -->
### p4-shellcheck-sc2015 · discovery · Local shellcheck 0.11.0 passed an SC2015 guard that CI's 0.9.0 failed (phase 4)

CI's lint job failed on 6b02f55: ubuntu-latest's shellcheck (0.9.0, per the orchestrator) reported SC2015 (info) on tests/run.sh's fakes preflight, [ ... ] && [ -x ... ] || { ...; exit 2; }. This host's shellcheck 0.11.0 passes the same line, even with --severity=style. Rewritten as an explicit if with the same message and exit 2; proven in a throwaway copy of tests/run.sh + tests/helpers (with the fakes: rc=0, 0/0 test files; gh fake removed: rc=2 with the unchanged refusal). tests/helpers/assert.sh's guard is already a case/if, and no other shell file phase 3 touched has the guard shape; the one remaining A && B || C outside pass/fail is tests/test_absorb.sh:505, an assignment that cannot fail, under that file's existing SC2015 disable and untouched by this plan. shellcheck -s bash -x --severity=style herdr-setup lib/*.sh tests/*.sh rc=0 and tests/helpers/*.sh rc=0 locally. Root cause (unpinned shellcheck) is removed in the next commit.

<!-- fr:journal kind=discovery scope=plan id=p4-shellcheck-pinned created=2026-09-14T12:55:25 phase=4 -->
### p4-shellcheck-pinned · discovery · shellcheck pinned through uv's dev group (shellcheck-py==0.11.0.1) (phase 4)

Root cause of p4-shellcheck-sc2015: shellcheck was the one unpinned gate (CI ubuntu-latest 0.9.0, this host 0.11.0). pyproject.toml's dev group adds shellcheck-py==0.11.0.1 (uv lock rc=0, uv.lock updated); .github/workflows/ci.yml's lint step runs uv run --group dev shellcheck -s bash -x herdr-setup lib/*.sh tests/*.sh, with the comment naming why; AGENTS.md's pinned-tools sentence and the plan prose's gate list name the pinned form. README.md names no shellcheck invocation. uv run --group dev shellcheck --version: 0.11.0; the pinned gate rc=0. Acceptance: no matrix row references the lint job or shellcheck (grep of docs/acceptance/matrix.yaml finds neither), so the matrix is unchanged. Every shellcheck gate in the rest of phase 4 uses the pinned command.

<!-- fr:journal kind=discovery scope=plan id=p4-red-t1s1 created=2026-09-14T12:57:30 phase=4 -->
### p4-red-t1s1 · discovery · RED: tests/test_audit_join.py before the sources and the join existed (P4.T1.S1) (phase 4)

First attempt did not count: the module-level feed = audit.feed raised AttributeError at import, before any test ran (a RED failing on its own setup proves nothing). The test now imports lib/feed.py by its own name, the module object lib/audit.py will import. Counted RED: uv run --quiet --script tests/test_audit_join.py -> rc=1, 20 tests, 33 errors (subtests counted), every one an AttributeError on a missing name (panes x11, pane_worktree x10, Gathered x7, load_adapters x5): TestPanes (every pane with agent, session value, cwd and tab label, calls exactly agent list then tab list; null agent_session -> None; unlisted tab -> None; empty only when herdr says so; FAKE_HERDR_FAIL on agent list and on tab list and an error object with exit 0 raise HerdrError; four unreadable answers raise), TestGatherSessions (only adapters declaring sessions are asked, with --since; --include-sdk only when asked; a failing adapter warned by name, recorded failed and incomplete while the other's sessions stay; every session malformed -> incomplete; a failing probe -> incomplete), TestWorktreePath (a path at, below or with a trailing slash under .cache/fr/worktrees/<repo>/<slug> names the branch and worktree; seven other paths name nothing), TestJoin (history branches; pane-cwd worktree-path branch added; an unsupported agent's pane shows its worktree branch; every section 1 note -- no branch evidence, session not reported to Herdr, session not found in history (outside --since, or SDK-driven), no session history (unsupported), no adapter; lookup under the pane's own agent; a failed adapter's pane says so; resolution entries cover every session and pane worktree once). Every HerdrCase test asserts a recording fr shim first on PATH was never run.

<!-- fr:journal kind=discovery scope=plan id=p4-green-t1s2 created=2026-09-14T12:59:55 phase=4 -->
### p4-green-t1s2 · discovery · GREEN: the sources and the join in lib/audit.py (P4.T1.S2), with two deviations (phase 4)

lib/audit.py imports lib/feed.py by name after sys.path.insert of its own directory, and gains warn, panes (agent list then tab list; an entry without a readable pane_id and agent, or a tab list without tabs, raises HerdrError; cwd falls back to foreground_cwd), load_adapters, Gathered, gather_sessions, pane_worktree (fullmatch of .../.cache/fr/worktrees/<repo>/<slug>[/...], slug __ -> /, re-filtered by feed.branch_name_ok), join and resolution_entries (each (name, dir, cwd) once). tests/test_audit_join.py rc=0, 20 tests. Deviation 1, a sixth section 1 note: a pane whose agent's adapter declared sessions but failed reads 'session history unavailable (adapter failed)' rather than 'session not found in history', which would be false; the run is incomplete anyway. Deviation 2: a failing PROBE is also an incomplete reason ('adapter <name>: probe failed: ...'), via a new optional skipped list on feed.usable_adapters, because a probe failure hides that agent's sessions and would label its panes 'no adapter' -- the spec's own reason an incomplete report exits 2. feed's own run still only warns.

<!-- fr:journal kind=discovery scope=plan id=p4-norefactor-t1s3 created=2026-09-14T12:59:57 phase=4 -->
### p4-norefactor-t1s3 · discovery · no-refactor-because: P4.T1.S3 (phase 4)

Each source is one function with one failure rule (panes raises HerdrError, gather_sessions records AdapterError as incomplete, pane_worktree is text), join is one pure pass per pane with the notes decided in one if-chain in the documented order, and resolution_entries is the only place entries are deduplicated. The notes are module constants shared with the report. Nothing duplicated to fold.

<!-- fr:journal kind=discovery scope=plan id=p4-red-t2s1 created=2026-09-14T13:10:28 phase=4 -->
### p4-red-t2s1 · discovery · RED: tests/test_audit_report.py before the report existed (P4.T2.S1) (phase 4)

uv run --quiet --script tests/test_audit_report.py -> rc=1, 24 tests, 33 errors (subtests counted), every one on a missing name: AttributeError build_report x18, exit_code x6, render_json x1, and TypeError main() got an unexpected keyword argument 'out' x8. TestSectionOne (one branch per (pane, repository branch) with evidence merged and sorted; gone hidden and counted, merged/contained/unmerged/unresolved shown, unresolved with its repository path and reason; a noted pane keeps its note and no branches), TestSectionTwo (unmerged and open-pr only, one row per (repo, branch), newest session with older_sessions; a branch an open pane's session touched excluded; the same name in two repositories two rows), TestSectionThree (a head matching a session branch in the base or the head repository excluded, head names case-sensitive, repositories not; bots hidden and counted; a deleted author not a bot; include_bots lists them), TestJsonShape (exactly the top-level keys in order and every nested key set; session ids whole), TestText (header with pane and session counts, since date and owners; three headings with the spec's columns; rows in the spec's shape with 8-character session ids and +N; columns aligned; (none) for an empty section; footer counts; incomplete line only when incomplete; no trailing whitespace; a pane whose every branch is gone keeps one row), TestExitCode (0, 1, 2 with incomplete winning over findings), TestRun (stage order require_tools, owners, panes, load_adapters, gather_sessions, join, resolve_branches, open_prs per owner; --json prints one object; ToolError, GhError, HerdrError, GitError and a GhError from open_prs each print one line and exit 2 with nothing on stdout; an unexpected exception exits 2 with its traceback, never 1).

<!-- fr:journal kind=discovery scope=plan id=p4-green-t2s2 created=2026-09-14T13:12:04 phase=4 -->
### p4-green-t2s2 · discovery · GREEN: build_report, render_text, render_json, exit_code, run and main (P4.T2.S2), with four named choices (phase 4)

lib/audit.py gains the report section (build_report pure over joined panes, sessions, Resolutions and pull requests; render_text and render_json both read only its object, so text and JSON cannot disagree; exit_code) and run(), wiring require_tools -> owners -> panes -> load_adapters/gather_sessions -> join -> resolve_branches -> open_prs per owner -> report; any AuditError or feed.HerdrError prints 'herdr-setup: audit: <message>' and returns 2 before stdout is touched. main(argv, *, out, err, now) replaces the phase 1 skeleton and its incomplete line; the module docstring lists every layer and the exit statuses. tests/test_audit_entrypoint.sh case (2) now expects the real runner to stop on the fake gh having no state: exit 2, stdout empty, exactly one stderr line naming gh api user. tests/test_audit_report.py rc=0 (24), tests/test_audit_join.py rc=0 (20), tests/test_audit_entrypoint.sh rc=0 (11 passed). Named choices where the spec is silent or its example is incomplete: (1) each open[].branches entry carries 'reason' (null unless unresolved) -- the spec's Resolving section says an unresolved branch is counted 'with the reason in JSON', which its JSON example has no key for; (2) a pane whose every branch is gone keeps one text row with state 'only gone branches (counted)' and JSON note null, branches [], so no pane Herdr lists is missing from section 1 (Test Plan step 3); (3) an empty section prints '  (none)' under its heading; (4) main() turns an exception run() did not anticipate into exit 2 with its traceback on stderr, because Python's own exit 1 would read as 'findings'. Also: merged branches show 'merged PR #N', open-pr 'open PR #N' plus ' (draft)'; section 2 is ordered newest session first, section 3 by repository then number; footer words agree in number ('1 branch gone', '1 bot PR hidden').

<!-- fr:journal kind=discovery scope=plan id=p4-norefactor-t2s3 created=2026-09-14T13:12:06 phase=4 -->
### p4-norefactor-t2s3 · discovery · no-refactor-because: P4.T2.S3 (phase 4)

build_report is one pass per section, each rule stated once (gone hidden in section 1, STRANDED and open-keys exclusion in section 2, base-or-head match and bot hiding in section 3); render_text shares one _table for all three sections and one _state_text for sections 1 and 2; the renderers read only the report object. run() is the only place stages are wired and the only error boundary. Nothing duplicated to fold.

<!-- fr:journal kind=finding scope=plan id=p4-matrix-yaml-colons created=2026-09-14T13:27:17 phase=4 state=fixed -->
### p4-matrix-yaml-colons · finding [fixed] · My matrix notes made matrix.yaml invalid YAML, so every fr command asked for a migration (phase 4)

Attributed to my own edit, not to a changed fr (fr 4.2.1, installed Sep 9, unchanged). After replacing the four session-audit rows, fr acceptance report and check, and then fr journal check, all exited 2 with 'artifacts ... were written for a different fr and must be migrated'. fr migrate artifacts without --yes (a dry run) named the cause: docs/acceptance/matrix.yaml was not valid YAML, because three plain-scalar notes lines contained a colon followed by a space ('gh: all three sections', 'the last clause: the audit', 'end to end: a squash-merged'), which YAML reads as a mapping; fr treats an unreadable artifact as unstamped. Nothing was migrated or committed. Fixed by rewording the three phrases with '--', with a check that no notes line in the four rows still contains ': '. fr acceptance report --deterministic rc=0, fr acceptance check rc=0 (13 rows: 12 ci, 1 skipped), fr journal check rc=0.

<!-- fr:journal kind=discovery scope=plan id=p4-t3-no-red-mutations created=2026-09-14T13:29:36 phase=4 -->
### p4-t3-no-red-mutations · discovery · Deviation: P4.T3.S1 could not be RED, so its assertions were proven by six mutations (phase 4)

Deviation from the plan text: P4.T3.S1 says run the suite -> rc non-zero, but T2's run() already wires every stage, so the new end-to-end block passed on its first run: /bin/bash tests/test_audit_entrypoint.sh rc=0, 50 passed, 0 failed, real 36.39 s (alone). A test that was never seen failing proves nothing, so each assertion group was proven by mutation instead: git archive of 0401712's successor b0787f2 (the T2 code, without the block) into a fresh copy, the block and tests/helpers/audit_e2e.py added, run unmutated (rc=0, 50/0), then one lib/audit.py mutation per fresh copy, replacement count asserted == 1; the worktree was only read. (1) exit_code ignores incomplete -> rc=1, 1 failed: an adapter that could not answer exits 2. (2) require_tools ignores a failing gh auth status -> rc=1, 4 failed: an unauthenticated gh exits 2, no report is printed, naming the reason, no GraphQL call was made. (3) bots never hidden -> rc=1, 2 failed: the bot pull request is hidden, the footer counts. (4) a branch an open pane touched no longer excluded from section 2 -> rc=1, 3 failed: the open pane's branch is not stranded, a clean state exits 0, section 2 is empty. (5) the text footer's gone count off by one -> rc=1, 2 failed: the footer, and the JSON check ('footer ... 2 branches gone ... disagrees with JSON counts gone 1'). (6) run() writes a file into CLAUDE_CONFIG_DIR -> rc=1, 2 failed: refs, status and store listings unchanged; no store file modified. Not mutated: 'gh saw no mutation' -- the fake gh refuses any query containing mutation before every switch, so no lib/audit.py mutation can produce one that answers. P4.T3.S2 needed no fix: nothing the end-to-end run exposed was wrong. Also added, beyond the step text: (5g) no Herdr server exits 3 before any gh call, because audit-fails-closed's row says 'unreachable', which case (1)'s protocol mismatch alone did not show.

<!-- fr:journal kind=discovery scope=plan id=p4-norefactor-t3s3 created=2026-09-14T13:29:38 phase=4 -->
### p4-norefactor-t3s3 · discovery · no-refactor-because: P4.T3.S3 (phase 4)

The block builds its sandbox once through tests/helpers/audit_e2e.py, runs every case through one audit_run helper that leaves rc and the two output files, and reads sections through one section helper; the fingerprint is taken once before and once after all runs. The JSON-versus-text comparison lives in audit_e2e.py check, the one place that parses both. Nothing duplicated to fold.

<!-- fr:journal kind=discovery scope=plan id=p4-t5-spec-drift created=2026-09-14T13:31:35 phase=4 -->
### p4-t5-spec-drift · discovery · Quality gate: the spec re-read against lib/audit.py, and the drift fixed in the spec (P4.T5.S1) (phase 4)

Re-read the spec's Commands (install, audit: order of work, text, JSON, exit codes), the sessions contract and The audit runner (Sources, Resolving, Merge state, GitHub queries, Bots) and Errors and safety against lib/audit.py, lib/feed.py and the entrypoint. Confirmed without change: hs_require_socket first (exit 3), then git and gh on PATH and gh auth status --hostname github.com --active (exit 2); session ids 8 characters in text and whole in JSON; warnings and errors on stderr, report on stdout; TAB is the tab label; every state but gone shown in section 1; section 2 unmerged and open-pr only, excluding branches an open pane's session touched, newest session plus +N; section 3 open pull requests in non-archived repositories, matched on base or head repository, bots hidden and counted unless --include-bots, drafts labelled; footer counts; incomplete line; JSON top-level keys; exit 0/1/2 with 2 winning; --dry-run and --yes accepted and change nothing; every adapter declaring sessions asked once with --since and --include-sdk; pane-directory worktree-path evidence; the audit never runs fr. Drift, all behaviour the code has that the spec did not state, fixed in the spec rather than the code: the JSON example's branch gains reason (null unless unresolved), with a sentence tying it to Resolving's 'with the reason in JSON'; section 1 gains the note 'session history unavailable (adapter failed)' and the one-row 'only gone branches (counted)' for a pane whose branches are all gone; the text output says every heading is printed and an empty section prints (none); Sources item 2 says a failing probe is also incomplete; the exit-code table says an unanticipated error is 2 with its traceback. The phase 1 skeleton's incomplete line and its docstring are gone.

<!-- fr:journal kind=discovery scope=plan id=p4-after-timing-under-load created=2026-09-14T13:55:54 phase=4 -->
