...
- **Evidence `command`.** Every `tool_use` block named `Bash`, from its
  ...
  - **Directory.** `cd <dir>` sets `dir` for the segments after it in the same
    command, and `git -C <dir>` sets it for its own segment. A relative path is
    resolved against the line's `cwd`, and `~` against `$HOME`, as text only.
  - **Shapes read:**
    - `fr isolation up|attach ... --branch <b>` (or `--branch=<b>`), with
      `--repo <path>` as `dir` when present;
    - `git checkout|switch -b|-c|-B <b>`;
    - `git worktree add <path> ... -b|-B <b>`, flags in any order, with `<path>`
      as `dir`;
...
- **Evidence `worktree-path`.** An fr worktree path,
  ...
  turned back into `/`, and `dir` is the worktree path up to and including the
  slug.
