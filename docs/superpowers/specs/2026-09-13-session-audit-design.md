...
- **Evidence `command`.** Every `tool_use` block named `Bash`, from its
  ...
  - **Directory.** `cd <dir>` sets the CURRENT directory for the segments
    ...
    directory (the latest `cd` already seen in this command, else the line's
    own `cwd`), and `~` resolves against `$HOME`, as text only. `cd -` and a
    bare `cd` leave the current directory unknown rather than inventing a
    path, falling back to the line's own `cwd`.
  - **Shapes read:**
